...
-- UNIFIED MASTER SETUP & FIX SCRIPT (V20 - ULTIMATE)
...
CREATE POLICY "Authenticated select activities" ON lead_activity_log FOR SELECT TO authenticated USING (true);
CREATE POLICY "Authenticated insert activities" ON lead_activity_log FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "Message insert" ON messages FOR INSERT WITH CHECK (sender_id = auth.uid());
CREATE POLICY "Participant view" ON message_participants FOR SELECT USING (user_id = auth.uid());
...
CREATE POLICY "Achievement view" ON agent_achievements FOR SELECT USING (true);
CREATE POLICY "Achievement definitions view" ON achievement_definitions FOR SELECT TO authenticated USING (true);
...
BEGIN
  ...
  IF FOUND AND p_current_value >= v_threshold THEN
    INSERT INTO agent_achievements (agent_id, achievement_id) VALUES (p_agent_id, v_achievement_id)
    ON CONFLICT (agent_id, achievement_id) DO NOTHING;
    IF FOUND THEN
        UPDATE agent_progress SET total_xp = total_xp + v_reward WHERE agent_id = p_agent_id;
    END IF;
  END IF;
END;
...
-- ========================================================
-- DONE. Artık tüm sistem hazır ve yetkiler tam uyumlu.
-- ========================================================
