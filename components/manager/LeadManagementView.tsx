...
export default function LeadManagementView() {
    ...
    const loadData = async () => {
        setLoading(true);
        // Load Agents
        const { data: agentsData } = await supabase.from('profiles').select('*').eq('role', 'agent');
        setAgents(agentsData || []);

        // Load Leads (Pending/Assigned)
        let query = supabase
            .from('leads')
            .select(`
                id, business_name, phone_number, status, assigned_to, created_at,
                profiles:assigned_to (full_name)
            `)
            .eq('status', 'pending'); // Only managing pending leads for now
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {loading ? (
                                ...
                                <tr><td colSpan={5} className="p-8 text-center text-gray-400">Gösterilecek lead bulunamadı.</td></tr>
                            ) : (
                                leads.map(lead => (
