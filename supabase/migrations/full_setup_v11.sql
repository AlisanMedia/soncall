...
-- FINAL COMPLETE SETUP SCRIPT (V11 - TEAM EDITION)
-- Includes: Core, Messaging, Reporting, Voice, Sales, Gamification, Appointments, AND Team Management
-- This script is designed to be IDEMPOTENT (runs safely multiple times without error)
-- ============================================
...
-- Re-create Core Policies
CREATE POLICY "Anyone can view profiles" ON profiles FOR SELECT USING (true);
-- UPDATED: Users can update their own profile OR Managers can update any profile (Team Management)
...
CREATE POLICY "Managers can view all leads" ON leads FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'manager'));
CREATE POLICY "Agents can view assigned leads" ON leads FOR SELECT USING (assigned_to = auth.uid());
CREATE POLICY "Managers can insert leads" ON leads FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'manager'));
CREATE POLICY "Managers can update leads" ON leads FOR UPDATE USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'manager'));
CREATE POLICY "Agents can update assigned leads" ON leads FOR UPDATE USING (assigned_to = auth.uid());
CREATE POLICY "Managers can view all batches" ON upload_batches FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'manager'));
CREATE POLICY "Managers can insert batches" ON upload_batches FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'manager'));

CREATE POLICY "Anyone can view notes" ON lead_notes FOR SELECT USING (true);
CREATE POLICY "Agents can insert notes for assigned leads" ON lead_notes FOR INSERT WITH CHECK (agent_id = auth.uid() AND EXISTS (SELECT 1 FROM leads WHERE leads.id = lead_id AND leads.assigned_to = auth.uid()));
...
CREATE POLICY "Managers can view all sales" ON sales FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'manager'));
CREATE POLICY "Managers can update sales" ON sales FOR UPDATE USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'manager'));
...
CREATE POLICY "Managers can manage all goals" ON public.goals FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'manager'));
...
-- Messaging Policies (Unified & Safe)
...
CREATE POLICY "unified_send_messages_policy"
  ON messages FOR INSERT
  WITH CHECK (sender_id = auth.uid());

CREATE POLICY "unified_update_messages_policy"
  ON messages FOR UPDATE
  USING (sender_id = auth.uid() OR receiver_id = auth.uid());

CREATE POLICY "unified_delete_messages_policy"
...
CREATE POLICY "Managers can view all call logs" ON public.call_logs FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'manager'));
...
-- ============================================
-- SETUP COMPLETE!
-- ArtificAgent System V11 - Team Edition
-- Features: Core, Messaging, Reporting, Voice, Sales, Gamification, Appointments, Team Management
-- ============================================
